...
release_code="b21a2ce"
active_module="/home/hongyi-jig-erp/odoo/staging_overrides/new_hongyijig_custom"
package_path="/home/hongyi-jig-erp/releases/incoming/Hongyi_Odoo_SSeries_b21a2ce.tar.gz"
expected_sha="bdec0c57a3cd86e4271748a8428354360b128ac19925fdfe51ee18cefac8c403"
expected_active_source_sha="0dda295e6e70436e4c1c202f8d7166a4e13fddef00d54f34ab17ce2fe0efdd64"
...
grep -q "0 failed, 0 error(s) of 198 tests" "/home/hongyi-jig-erp/releases/work/${release_code}/post_install_tests.log"
grep -q "SSERIES_WORKFLOW_PASS" "/home/hongyi-jig-erp/releases/work/${release_code}/sseries_validation.log"
grep -q "'version': '19.0.3.20.2'" "${active_module}/__manifest__.py"
...
grep -q "'version': '19.0.3.21.0'" "${candidate_root}/new_hongyijig_custom/__manifest__.py"
...
for field_name in (
    ...
    if field_name not in env["hjig.sseries.intake.attachment.gateway"]._fields:
        raise RuntimeError(f"Missing malware-scanning evidence field: {field_name}")
for field_name in (
...
if module.installed_version != "19.0.3.21.0":
    raise RuntimeError(f"Unexpected installed version: {module.installed_version}")
print(
    "STAGING_SSERIES_WORKFLOW_PASS version=19.0.3.21.0 "
...
deployment_started=0
trap - ERR
echo "HJIG_STAGING_UPGRADE_SUCCESS release=${release_code} version=19.0.3.21.0 rollback=${rollback_root} http=${http_code} production=untouched"
