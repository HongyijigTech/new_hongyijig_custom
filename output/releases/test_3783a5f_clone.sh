...
release_code="3783a5f"
package_path="/home/hongyi-jig-erp/releases/incoming/Hongyi_Odoo_SSeries_3783a5f.tar.gz"
expected_sha="1221a2a5d83ff0fcb0211080bcf69cdfd65ea1d89311dfb214be04a78d1126b0"
...
test_database="hongyijig_sseries_test_3783a5f"
...
if test_database != "hongyijig_sseries_test_3783a5f":
...
for case in cases.filtered(lambda item: item.stage == "b0_released"):
    ...
    for code, reference in expected_evidence.items():
        if not (reference or "").strip():
            continue
        artifact = case.artifact_ids.filtered(lambda item, value=code: item.code == value)[:1]
        if not artifact or artifact.state != "evidence_recorded":
            raise RuntimeError(f"External evidence status was not reconciled for {case.name}/{code}")
        if artifact.issue_reference != reference:
            raise RuntimeError(f"External evidence reference mismatch for {case.name}/{code}")
        if artifact.customer_issue_allowed or artifact.supplier_issue_allowed:
            raise RuntimeError(f"External evidence must not create issue authority for {case.name}/{code}")
for field_name in (
...
print(
    "SSERIES_AUTHORITY_PASS version=19.0.3.27.0 templates=24 activation_records=13 "
