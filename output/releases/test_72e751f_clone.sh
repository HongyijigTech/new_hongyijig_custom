...
test_database="hongyijig_sseries_test_72e751f"
...
if test_database != "hongyijig_sseries_test_72e751f":
...
grep -q "0 failed, 0 error(s) of 170 tests" "${test_log}"
...
print("SSERIES_WORKFLOW_PASS version=19.0.3.14.0 templates=22 cases=0 bseries_preserved=true production_untouched=true")
env.cr.rollback()
PY
...
test "$(systemctl is-active odoo.service)" = "active"
test "$(systemctl is-active odoo-production.service)" = "active"
echo "HJIG_CLONE_TEST_PASS release=${release_code} tests=170 failures=0 errors=0 production_untouched=true"
