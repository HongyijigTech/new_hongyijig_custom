...
release_code="29c127b"
active_module="/home/hongyi-jig-erp/odoo/staging_overrides/new_hongyijig_custom"
package_path="/home/hongyi-jig-erp/releases/incoming/Hongyi_Odoo_SSeries_29c127b.tar.gz"
expected_sha="2ae152de77ff967103a8cca6336b08548cc26198d7cb385daa23745b07c4aa16"
expected_active_source_sha="b48b4774a689a010a5366ec2665c3e9d5881b2a0664458d766208642dbdf8368"
...
grep -Eq "0 failed, 0 error\(s\) of [0-9]+ tests" "/home/hongyi-jig-erp/releases/work/${release_code}/post_install_tests.log"
grep -q "SSERIES_AUTHORITY_PASS" "/home/hongyi-jig-erp/releases/work/${release_code}/sseries_validation.log"
# Reconciled exact staging state discovered on 2026-08-31 after a parallel
# 19.0.3.22.6 file-level replacement.  The full source-tree digest above is
# the fail-closed authority; the candidate restores every subsequently tested
# S-Series/NDA control while retaining the exact governed template payloads.
grep -q "'version': '19.0.3.22.6'" "${active_module}/__manifest__.py"
...
grep -q "'version': '19.0.3.28.0'" "${candidate_root}/new_hongyijig_custom/__manifest__.py"
printf '%s  %s\n' \
...
if module.installed_version != "19.0.3.28.0":
...
if "evidence_recorded" not in dict(Artifact._fields["state"].selection):
    raise RuntimeError("External evidence terminal state is missing")
for case in cases.filtered(lambda item: item.stage == "b0_released"):
    expected_external = {
        "S5-PAYMENT-EVIDENCE": case.payment_evidence_reference,
        "S5-TAX-INVOICE": case.tax_invoice_reference,
        "B0-HANDOVER-MANIFEST": case.b0_manifest_id.name,
    }
    for artifact_code, reference in expected_external.items():
        if not reference:
            raise RuntimeError(f"{case.reference}: missing external evidence reference for {artifact_code}")
        artifact = case.artifact_ids.filtered(lambda item: item.code == artifact_code)
        if len(artifact) != 1 or artifact.state != "evidence_recorded":
            raise RuntimeError(f"{case.reference}: external evidence state not reconciled for {artifact_code}")
        if artifact.issue_reference != reference or artifact.customer_issue_allowed or artifact.supplier_issue_allowed:
            raise RuntimeError(f"{case.reference}: unsafe external evidence record for {artifact_code}")
...
print(
    "STAGING_SSERIES_AUTHORITY_PASS version=19.0.3.28.0 templates=24 activation_records=13 "
    f"submissions={env['hjig.sseries.intake.submission'].search_count([])} cases={len(cases)} "
    "core_internal_uat_renderers=4 one_crm_spine=true bseries_preserved=true"
)
env.cr.rollback()
PY
grep -q "STAGING_SSERIES_AUTHORITY_PASS" "${postcheck_log}"

grep -q "'version': '19.0.3.28.0'" "${active_module}/__manifest__.py"
sudo -n /usr/bin/systemctl start odoo.service
test "$(systemctl is-active odoo.service)" = "active"
test "$(systemctl is-active odoo-production.service)" = "active"
test "$(systemctl is-active clamav-daemon.service)" = "active"
...
deployment_started=0
trap - ERR
echo "HJIG_STAGING_UPGRADE_SUCCESS release=${release_code} version=19.0.3.28.0 rollback=${rollback_root} http=${http_code} production=untouched"
