...
test_database="hongyijig_sseries_test_77d4ea9"
...
if test_database != "hongyijig_sseries_test_77d4ea9":
...
grep -q "0 failed, 0 error(s) of 169 tests" "${test_log}"
...
print("SSERIES_WORKFLOW_PASS version=19.0.3.13.0 templates=22 cases=0 bseries_preserved=true production_untouched=true")
env.cr.rollback()
PY
...
test "$(systemctl is-active odoo.service)" = "active"
test "$(systemctl is-active odoo-production.service)" = "active"
echo "HJIG_CLONE_TEST_PASS release=${release_code} tests=169 failures=0 errors=0 production_untouched=true"
