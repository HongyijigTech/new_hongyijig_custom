...
release_code="4f53e3d"
active_module="/home/hongyi-jig-erp/odoo/staging_overrides/new_hongyijig_custom"
package_path="/home/hongyi-jig-erp/releases/incoming/Hongyi_Odoo_SSeries_4f53e3d.tar.gz"
expected_sha="915c283e4ceee3a4a9e2c55a9e447b57598d8aebb7ab07bf981b6fb31a9bdb92"
expected_active_source_sha="f413250ab505382e19f0f32d804295b381196aef5b90612612e74bb07af2ed15"
...
grep -q "0 failed, 0 error(s) of 173 tests" "/home/hongyi-jig-erp/releases/work/${release_code}/post_install_tests.log"
grep -q "SSERIES_WORKFLOW_PASS" "/home/hongyi-jig-erp/releases/work/${release_code}/sseries_validation.log"
grep -q "'version': '19.0.3.14.0'" "${active_module}/__manifest__.py"
...
grep -q "'version': '19.0.3.15.0'" "${candidate_root}/new_hongyijig_custom/__manifest__.py"
...
if module.installed_version != "19.0.3.15.0":
    raise RuntimeError(f"Unexpected installed version: {module.installed_version}")
print(
    "STAGING_SSERIES_WORKFLOW_PASS version=19.0.3.15.0 "
...
grep -q "'version': '19.0.3.15.0'" "${active_module}/__manifest__.py"
sudo -n /usr/bin/systemctl start odoo.service
test "$(systemctl is-active odoo.service)" = "active"
test "$(systemctl is-active odoo-production.service)" = "active"
http_code="000"
...
deployment_started=0
trap - ERR
echo "HJIG_STAGING_UPGRADE_SUCCESS release=${release_code} version=19.0.3.15.0 rollback=${rollback_root} http=${http_code} production=untouched"
